...
An example report (corresponding to the second example below) is available for examination at \url{http://starkhome.com/ChIPQC/Reports/tamoxifen/ChIPQC.html}.
...
NB: currently, only "hg19" is supported, with more to follow.
...
By default, the "hg19" annotation will be used with a default blacklist, and only the first chromosome (in this case "chr22") will be examined.
...
This report is available at \url{http://starkhome.com/ChIPQC/Reports/exampleExp/ChIPQC.html}. 
...
data(blacklist_hg19)
tamoxifen = ChIPQC(samples, consensus=TRUE, bCount=T, summits=250,
...
Next the read length, derived from the data in the bam files, is reported, along with the estimated mean fragment length. 
The fragment length is estimated from the data by systematically shifting the reads on each strand towards each other until the highest degree of cross-correlation is obtained.
The \Rcode{RelativeCC} metric is calculated by comparing the maximal cross correlation peak (at the shift size corresponding to he fragment length) to the cross correlation at a shift size corresponding to the read length, with higher scores (generally 2 or greater) indicating good enrichment.
...
The resulting report is not included in this vignette due to space limitations, but you are invited to generate it yourself and have a look. It is also available for examination at \url{http://starkhome.com/ChIPQC/Reports/tamoxifen/ChIPQC.html}.
...
@
\incfig{ChIPQC-figCC}{.66\textwidth}{Example Cross-correlation plot.}
  {Generated by \Rcode{plotCC(tamoxifen,facetBy=c("Tissue","Condition"))}}
  
As shown in Figure~\ref{ChIPQC-figCC}, this plots the cross-correlation values over a range of shift sizes. The region up to the read length is highlighted, and a small peak in cross-correlation is expected to be found here. Notice that the controls do not have another higher peak, while ChIP samples with good enrichment have another peak in cross-correlation value at the fragment length, as shifting the reads on both strand should increase correlation at peak sites.
...
@
\incfig{ChIPQC-figCCBySample}{.66\textwidth}{Cross-correlation plot grouped by Sample ID.}
...
@
\incfig{ChIPQC-figCCColourByFlineByTissue}{.66\textwidth}{Cross-correlation plot grouped by Sample ID.}
...
@
\incfig{ChIPQC-figCCaddMetadata}{.66\textwidth}{Cross-correlation plot grouped by Sample ID.}
